...
const BottomNav: React.FC = () => {
  ...
  return (
    <nav 
      className="shrink-0 max-w-[430px] mx-auto bg-synapse-deep/90 border-t border-synapse-border px-2 pt-2 flex justify-around items-start z-50 backdrop-blur-2xl"
      style={{ 
        ...
        return (
          ...
          >
            <div className={`w-12 h-8 flex items-center justify-center rounded-full transition-all duration-300 ${
              ...
              <i className={`fa-solid ${item.icon} text-lg transition-transform ${isActive ? 'scale-110 drop-shadow-[0_0_8px_rgba(45,212,191,0.5)]' : 'scale-100'}`}></i>
            </div>
            <span className={`text-[9px] tracking-widest uppercase transition-all ${isActive ? 'font-black' : 'font-bold opacity-50'}`}>
              {item.label}
            </span>
          </Link>
        );
