...
const Dashboard: React.FC = () => {
  ...
  return (
    <div className="pb-40 min-h-screen relative overflow-x-hidden bg-synapse-light-bg">
      ...
      {/* Header with standardized safe padding - SOFT BLUE THEME */}
      <div className="pt-12 px-8 pb-16 relative rounded-b-[40px] overflow-hidden">
        <div className="absolute top-8 right-8 flex gap-3 z-20">
           <button 
             onClick={() => navigate('/notifications')}
             className="w-12 h-12 bg-white/80 backdrop-blur-md rounded-full shadow-sm flex items-center justify-center text-synapse-blue-primary border border-white active:scale-90 transition-all"
           >
              <i className="fa-solid fa-bell"></i>
              {unreadNotifs > 0 && (
                <div className="absolute top-2.5 right-2.5 w-3.5 h-3.5 bg-synapse-error text-white text-[8px] font-black rounded-full flex items-center justify-center border-2 border-white">
                  {unreadNotifs}
                </div>
              )}
           </button>
        ...
        <h1 className="text-4xl font-light text-synapse-text-primary leading-tight tracking-tight uppercase relative z-10">Ready,<br/><span className="font-black text-synapse-blue-primary tracking-tighter">Dr. {userData?.name?.split(' ')[1] || 'Sarah'}</span></h1>
      </div>
      ...
      <div className="px-5 space-y-8 pb-12 -mt-8">
        ...
        {hasWeakness && (
          <div 
            ...
            className="bg-white rounded-samsung p-5 flex items-center gap-4 animate-in slide-in-from-top duration-500 cursor-pointer active:scale-[0.98] transition-all border border-red-100 shadow-sm"
          >
            ...
            <div className="flex-1">
              ...
              <p className="text-xs font-bold text-synapse-text-primary">Focused Improvement Recommended</p>
            </div>
          ...
          <div onClick={() => navigate('/analytics')} className="bg-white rounded-samsung p-7 shadow-xl border border-white active:scale-[0.98] transition-all cursor-pointer space-y-6 card-shadow">
            <div className="flex justify-between items-start">
              <div className="space-y-1">
                ...
                <p className="text-5xl font-black text-synapse-text-primary tracking-tighter">78%</p>
                <div className="flex items-center gap-1.5 text-synapse-blue-primary font-black text-[9px] uppercase tracking-widest pt-2">
                  <i className="fa-solid fa-bolt"></i>
                  <span>Accelerating Prep</span>
                </div>
              </div>
              <div className="text-right space-y-1">
                <p className="text-[9px] font-black text-synapse-text-secondary uppercase tracking-widest">Global Ranking</p>
                <p className="text-xl font-black text-synapse-text-primary">#1,242</p>
                <p className="text-[9px] font-black text-synapse-blue-primary uppercase">Top 2%</p>
              </div>
            </div>
            <div className="w-full h-12 flex items-end gap-2">
               {[40, 65, 45, 80, 55, 90, 78].map((h, i) => (
                 <div key={i} className={`flex-1 rounded-t-lg transition-all duration-700 ${i === 6 ? 'bg-synapse-blue-primary shadow-[0_0_15px_rgba(59,130,246,0.2)]' : 'bg-slate-50'}`} style={{ height: `${h}%` }} />
               ))}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-2">
            <h3 className="text-[10px] font-black text-synapse-text-secondary uppercase tracking-[0.2em]">Today Image</h3>
          </div>
          <div 
            onClick={() => setIsImageViewOpen(true)}
            className="bg-white rounded-samsung p-4 border border-white shadow-lg relative overflow-hidden active:scale-[0.98] transition-all cursor-pointer group card-shadow"
          >
            <div className="relative aspect-[16/10] w-full rounded-2xl overflow-hidden bg-slate-50">
              <img 
                src={dailyImage.url} 
              ...
              <div className="absolute top-4 left-4">
                <span className="bg-white/90 backdrop-blur-md border border-white text-synapse-blue-primary text-[9px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest shadow-sm">
                  {dailyImage.tag}
                </span>
              </div>
            </div>
            <div className="mt-5 px-2 space-y-2">
              <h4 className="text-lg font-black text-synapse-text-primary leading-tight">{dailyImage.caption}</h4>
              <p className="text-xs text-synapse-text-secondary font-medium line-clamp-1 leading-relaxed">
                {dailyImage.description}
              </p>
              <button className="w-full h-12 mt-2 bg-synapse-blue-primary text-white rounded-xl font-black text-[10px] uppercase tracking-[0.3em] hover:bg-synapse-blue-secondary transition-all">
                View Case
              </button>
            </div>
          </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-2">
            <h3 className="text-[10px] font-black text-synapse-text-secondary uppercase tracking-[0.2em]">High Yield Flash</h3>
          </div>
          <div className="bg-white rounded-samsung p-6 border-l-4 border-synapse-blue-primary shadow-xl active:scale-[0.98] transition-all group card-shadow">
             <div className="flex justify-between items-center mb-4">
                <h4 className="text-xs font-black text-synapse-blue-primary uppercase tracking-[0.2em]">{dailyFlash.title}</h4>
                <div className="w-8 h-8 bg-synapse-blue-primary/10 rounded-lg flex items-center justify-center text-synapse-blue-primary border border-synapse-blue-primary/10">
                   <i className="fa-solid fa-bolt-lightning text-xs"></i>
                </div>
             </div>
             <div className="max-h-32 overflow-y-auto no-scrollbar mb-6">
                <p className="text-sm font-medium text-synapse-text-primary leading-relaxed">
                  {dailyFlash.concept}
                </p>
             </div>
             <div className="flex items-center justify-between pt-4 border-t border-slate-50">
                <span className="text-[9px] font-black text-synapse-text-secondary uppercase tracking-widest">{dailyFlash.tag}</span>
                <button className="text-synapse-blue-primary text-[10px] font-black uppercase tracking-[0.2em] flex items-center gap-2 group-hover:translate-x-1 transition-transform">
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-center px-2">
            <h3 className="text-[10px] font-black text-synapse-text-secondary uppercase tracking-[0.2em]">Live Stream</h3>
          </div>
          <div onClick={() => navigate('/live-list')} className="bg-white rounded-samsung p-7 shadow-2xl relative overflow-hidden active:scale-[0.98] transition-all cursor-pointer group border border-red-100 card-shadow">
            <div className="absolute top-0 right-0 w-40 h-40 bg-red-50 rounded-full -mr-20 -mt-20 blur-3xl animate-pulse" />
            <div className="flex justify-between items-start relative z-10 mb-6">
              ...
              <div className="w-10 h-10 bg-slate-50 text-synapse-blue-primary rounded-xl flex items-center justify-center border border-slate-100">
                 <i className="fa-solid fa-satellite-dish"></i>
              </div>
            </div>
            <h4 className="text-xl font-black text-synapse-text-primary leading-tight mb-2">Neuroanatomy Masterclass</h4>
            <p className="text-xs text-synapse-text-secondary font-medium mb-6">Dr. Anand Sharma • 1.2k Connected</p>
            <button className="w-full h-14 bg-synapse-blue-primary text-white rounded-2xl font-black text-[10px] uppercase tracking-widest active:bg-synapse-blue-secondary transition-all">
              Secure Join
            </button>
          </div>
        </div>
