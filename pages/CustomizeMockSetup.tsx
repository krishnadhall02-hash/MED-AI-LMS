  ...
  return (
    <div className="pb-40 bg-oneui-bg min-h-screen">
      <div className="oneui-header-space flex flex-col justify-end px-8 pb-8">
        ...
        <h1 className="text-4xl font-light text-slate-900 leading-tight">Customize<br/><span className="font-bold">Mock Test</span></h1>
      </div>

      <div className="px-5 space-y-6 -mt-4">
        {/* Real-time Preview Card */}
        <div className="bg-slate-900 rounded-samsung p-8 text-white shadow-xl relative overflow-hidden">
           <div className="absolute top-0 right-0 w-32 h-32 bg-oneui-blue/20 rounded-full -mr-16 -mt-16 blur-3xl"></div>
           <div className="flex justify-between items-start relative z-10">
              <div className="space-y-1">
                 <p className="text-[10px] font-black text-oneui-blue uppercase tracking-widest">Test Pace</p>
                 <p className="text-4xl font-black">{secPerQuestion} <span className="text-lg">sec/q</span></p>
              </div>
              {isSpeedMode && (
                <span className="bg-amber-500 text-white text-[9px] font-black px-3 py-1.5 rounded-full uppercase tracking-widest animate-pulse shadow-lg">
                  Speed Mode
                </span>
              )}
           </div>
           <p className="text-xs font-medium opacity-60 mt-4 leading-relaxed relative z-10">
             {isSpeedMode 
        ...
        <div className="bg-white rounded-samsung p-8 shadow-sm border border-slate-100 space-y-10">
          
          {/* Question Count */}
          <div className="space-y-4">
             <div className="flex justify-between items-center">
                <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Questions</p>
                <span className="text-xl font-black text-slate-900">{questionCount}</span>
             </div>
             <input 
               type="range" 
               min="10" 
               ...
               className="w-full h-2 bg-slate-100 rounded-full appearance-none accent-oneui-blue"
             />
          ...
          <div className="space-y-4">
             <div className="flex justify-between items-center">
                <div className="flex items-center gap-2">
                   <p className="text-xs font-black text-slate-400 uppercase tracking-widest">Time Limit</p>
                   {isSpeedMode && <i className="fa-solid fa-bolt text-amber-500 text-[10px]"></i>}
                </div>
                <div className="text-right">
                   <span className="text-xl font-black text-slate-900">{timeLimit} <span className="text-sm">mins</span></span>
                </div>
             </div>
             <input 
               type="range" 
             ...
             <div className="flex justify-between text-[9px] font-black text-slate-300 uppercase tracking-tighter">
                <span>10 mins</span>
                <span className="text-oneui-blue">Limit: {questionCount} mins (max)</span>
             </div>
                ...
                {['Mixed', 'Easy', 'Medium', 'Hard'].map((d) => (
                  <button
                    ...
                    className={`h-14 rounded-2xl border-2 font-bold text-xs uppercase tracking-widest transition-all ${
                      difficulty === d ? 'border-oneui-blue bg-blue-50 text-oneui-blue shadow-sm' : 'border-slate-50 text-slate-400'
                    }`}
                  >
                    {d}
                  </button>
        ...
        <div className="pt-4">
           ...
           <button 
             onClick={handleStart}
             className="w-full h-16 bg-slate-900 text-white rounded-samsung font-black text-xl shadow-xl shadow-slate-200 active:scale-95 transition-all"
           >
             Create & Start Test
           </button>
        </div>
