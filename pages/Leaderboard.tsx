  ...
  return (
    <div className="pb-40 bg-oneui-bg min-h-screen">
      <div className="oneui-header-space flex flex-col justify-end px-8 pb-8">
        <button onClick={() => navigate(-1)} className="w-10 h-10 -ml-2 text-oneui-text-secondary mb-2">
          <i className="fa-solid fa-arrow-left text-xl"></i>
        </button>
        <h1 className="text-4xl font-light text-oneui-text-primary leading-tight">Monthly<br/><span className="font-bold">Scholarship</span></h1>
      </div>

      <div className="px-5 space-y-8 -mt-4">
        {/* Scholarship Reward Callout */}
        <div className="bg-slate-900 rounded-[32px] p-8 text-white relative overflow-hidden shadow-2xl">
           <div className="absolute top-0 right-0 w-32 h-32 bg-oneui-blue/20 rounded-full -mr-16 -mt-16 blur-3xl animate-pulse" />
           <div className="flex items-center gap-5 relative z-10">
              <div className="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center text-3xl backdrop-blur-md border border-white/10">
                 <i className="fa-solid fa-graduation-cap text-oneui-blue"></i>
              </div>
              <div>
                 <h3 className="text-xl font-black leading-tight">Monthly Reward</h3>
                 <p className="text-[10px] font-black text-oneui-blue uppercase tracking-widest mt-1">₹10,000 for Top 3</p>
              </div>
           </div>
           <p className="text-xs font-medium text-white/70 mt-6 leading-relaxed relative z-10">
        ...
        <div className="flex items-end justify-center gap-4 pt-10 pb-4">
           ...
           <div className="flex flex-col items-center gap-3">
              ...
              <div className="w-16 h-20 bg-oneui-surface rounded-t-2xl shadow-sm border border-oneui-border flex items-center justify-center">
           ...
           <div className="flex flex-col items-center gap-3">
              ...
              <div className="w-20 h-28 bg-oneui-surface rounded-t-2xl shadow-xl border-t-4 border-t-amber-400 flex items-center justify-center">
           ...
           <div className="flex flex-col items-center gap-3">
              ...
              <div className="w-14 h-16 bg-oneui-surface rounded-t-2xl shadow-sm border border-oneui-border flex items-center justify-center">
        ...
        <div className="space-y-3">
           {LEADERS.map(user => (
             <div key={user.rank} className={`bg-oneui-surface p-5 rounded-samsung border border-oneui-border flex items-center gap-4 transition-all ${user.rank === 4 ? 'ring-2 ring-oneui-blue ring-offset-2' : ''}`}>
                <span className="w-6 text-center text-xs font-black text-oneui-text-secondary">{user.rank}</span>
                <img src={user.avatar} className="w-10 h-10 rounded-full shadow-inner shrink-0" alt="" />
                <div className="flex-1">
                   <h4 className="font-black text-oneui-text-primary text-sm">{user.rank === 4 ? 'You (Sarah)' : user.name}</h4>
                   <p className="text-[10px] font-bold text-oneui-text-secondary uppercase tracking-widest">{user.accuracy}% Accuracy • {user.time}</p>
                </div>
                {user.rank <= 3 && (
        ...
        <div className="bg-oneui-bg p-6 rounded-samsung border border-dashed border-oneui-border text-center">
           <p className="text-[10px] font-black text-oneui-text-secondary uppercase tracking-widest">Next reset in 12 days</p>
        </div>
      </div>
    </div>
  );
